...
`/tmp/k7-xts-longrun/serial.log`。没有 24 小时终点记录，1.3.14
本轮不能判 PASS。恢复串口后重新设定起点并完成整段 24 小时测试。
...
| 1.3.14 时间一致性 | 本轮约 15h 串口断连，无 24h 终点 | 串口恢复后重跑完整 24h，核对四次采样、终点与起点的相对漂移（≤2 秒）；设时命令伴随 `exec failed: 2`，需查明是否违反“无报错”。 |
