...
当前 24 小时长测自 2026-09-15 00:12:02 新加坡时间开始，预计 2026-09-16 00:12:02 结束。在确认终点数据前，不刷机、重启、抢占串口或运行以下复测。
...
| 1.3.4 RAM 随机读写 | `mkrd -m 10 -s 512 2048`，block 3/3 | 原文示例为 `mkrd -m 10 -s 1000 1024`；核对参数差异和对应设备，再按原步骤留证。 |
| 1.3.5 Flash 功能 | SD 卡 FAT32 上 `fstest` 20/20 | 原文要求 Flash 设备上的 `cmocka_driver_block -m <设备>`；先确认测试会否破坏数据，再选可安全测试的介质。现结果只作替代功能验证。 |
| 1.3.7 I²C/SPI | RTC/触摸 I²C 正常，SPI 时钟自检 | 原版要求接 BMI160、一次选 I²C 或 SPI、运行 `cmocka_driver_i2c_spi`。MPU6050 仅可做 I²C 替代验证；若需正式替代器件，按原文向社区确认。另一块开发板并非这条原版用例的前提。 |
| 1.3.12 RTC | HYM8563 读写及掉电保持 | 原文要求 `cmocka_driver_rtc`；补命令与完整结果，硬件测试保留作辅助证据。 |
| 1.3.14 时间一致性 | 24 小时长测中 | 到 2026-09-16 00:12:02 新加坡时间后核对四次采样、终点与起点的相对漂移（≤2 秒）；设时命令伴随 `exec failed: 2`，需查明是否违反“无报错”。 |
...
| 3.1.1 12h 待机 | 当前 12h 观察中 | 当前镜像无 `CONFIG_MM_KASAN`，即使观察稳定也不能按原文判 PASS。长测结束后编译/烧录 KASAN + show_info 镜像，再静置 12h 留全程日志。 |
