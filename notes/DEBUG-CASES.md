...
## 案例 15：自配窗口出斜纹 —— 先找一个"已知正确"的原点（进行中）
...
诊断来自读代码，"读代码得出的结论"和"在这块板上成立"是两件事。
`cam morph 7` 专门把那个错误值写回去：**6 出正的竖条纹、7 出斜纹**，
两次对照才算证实，否则只能说"改完好了"，说不清是不是别的什么被一起
改掉了。案例 12 在这条链路上已经吃过一次这样的亏。
