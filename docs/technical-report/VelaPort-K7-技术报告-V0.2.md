...
实测结果：网络 100/100 包成功、平均时延 0.85 ms；相机预览约 30 fps；从 `reboot` 到 NuttShell 平均 2.76 s、冷启动平均 2.84 s（各 10 次）；12 小时待机（KASAN 镜像）无重启、无异常、堆无泄漏。xTS 通用自测 35 项中 **29 项**按原文口径复测通过并保留原始日志；未达成的 4 项各自给出了原因，不做推测性判定。
...
| 档 | 数量 | 含义 |
| --- | --- | --- |
| A 严格通过 | **29** | 按原文步骤执行，保留原始串口/网络字节与镜像标识，判据未改 |
| B 部分通过 | **1** | 1.3.11 UART 文件传输：主机→板通过（md5 一致），板→主机受 1.5 Mbaud 下主机侧丢字（实测 7~16%）限制 |
| C 未达成 | **5** | 1.1.4 ostest（卡在多线程锁子项，根因是 CPU1 在双系统下不接任务）、1.3.5、1.3.7、1.3.14、1.3.15 |

2026-09-20 当天把 B 档按原文重跑并补测：14 项升入 A，另有 1.3.1 烧写、1.3.6 GPIO 通过，其中 1.3.6 顺带修出一个 GPIO 驱动缺陷（输出脚读回读了 EXT_PORT，而本板该脚做输出时输入缓冲不工作，改读输出数据寄存器后 4/4）；1.1.4 ostest 则暴露为未通过，根因是既有的 CPU1 缺陷。
