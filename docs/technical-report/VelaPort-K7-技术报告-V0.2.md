...
V0.2 相对 V0.1（2026-09-17）的变化：云端推理与语音闭环已打通并留有实测；启动时间二次优化；xTS 按原文口径完成复测，缺口从 16 项收敛到 3 项，并逐项给出未达成的原因。
...
本作品交付的是一套平台而非单个应用：将 openvela 适配至 KICKPI-K7（RK3576），并在同一颗 SoC 上划出"实时域 + 计算域"——A53 四核运行 openvela 掌管外设与交互，A72 四核运行 Linux 作为可扩展的计算域（厂商 NPU/ISP 软件栈只有 Linux 版）。桌面视觉助手是验证载体，用来把这套结构的每一条缝串起来跑通，换一个应用形态时下层不需重写。实现启动、中断与外设适配，接入 LVGL、IMX415 相机、音频与 ai_agent，形成可用的桌面视觉助手：说出唤醒词后，设备完成识别、拍照、视觉问答与语音播报的完整链路，其中语音端点检测由 A72 的 Linux 承担、openvela 负责外设与交互。关键工作包括共享 GIC 资源划分、DMA 缓冲管理及烧写布局校验。

实测结果：网络 100/100 包成功、平均时延 0.85 ms；相机预览约 30 fps；从 `reboot` 到 NuttShell 平均 2.76 s、冷启动平均 2.84 s（各 10 次）；12 小时待机（KASAN 镜像）无重启、无异常、堆无泄漏。xTS 通用自测 35 项中 **31 项**按原文口径复测通过并保留原始日志；未达成的 3 项各自给出了原因，不做推测性判定。
...
**先说清楚定位：桌面助手是验证载体，不是作品本身。** 本作品的对象是一套
平台——把 openvela 适配到 RK3576，并在同一颗 SoC 上划出"实时域 + 计算域"
的双系统结构。桌面助手（看桌面、找钥匙和水杯、巡检提醒）之所以存在，是因为
它恰好把这套结构的每一条缝都串了一遍：触摸与显示、相机采集、音频输入输出、
核间通信、网络与模型调用。换一个应用（工业 HMI、车载仪表、带屏家电），
下面这套东西不用重写。因此本节讨论的是平台要解决什么问题，而不是这个 demo
能识别几种物品。

**为什么要两个操作系统。** 带屏终端的需求天然分成互相冲突的两半：

| 一半 | 要求 | 谁来做 |
| --- | --- | --- |
| 交互与外设 | 确定性：触摸到出画要稳、音频采集不能丢帧、看门狗要能救、开机要快 | openvela（A53×4）：RTOS 调度可预期，代码体量小、启动 2.8 s，外设驱动自己掌握 |
| 智能与算力 | 生态：模型运行时、NPU/ISP 厂商软件栈、文件系统与包管理 | Linux（A72×4）：`librknnrt`、`rkaiq` 只有 Linux 版，移植代价远大于让它跑在另一个簇上 |

用纯 Linux 做，交互确定性和启动时间要靠调参去凑，而且这块板子在 openvela
上本来就没有适配代码，赛题本身就落空；用纯 RTOS 做，等于把厂商的 NPU/ISP
软件栈从零重写。RK3576 的 A53 + A72 异构布局恰好允许两者并存：**openvela
是产品主体、默认拥有一切外设；Linux 只拿它非拿不可的那几条链**（见
`amp/OWNERSHIP.md`）。这个判断不是从架构图推出来的，是从 3A 闭环这类具体
约束反推的——曝光增益要按帧经 I2C 写回 sensor，把 sensor 的 I2C 放到对端
就等于给一个时序敏感的闭环塞进核间往返，所以整条相机硬件链留在 openvela。

**端侧 Agent 为什么落在 Linux 侧。** 真正想做的形态是：openvela 负责"感知与
呈现"，Linux 负责"理解与决策"，两者用 rpmsg 连起来；模型能换、能升级、能离线，
而交互层不受影响。本作品已经把这条缝跑通并且放了**真实负载**而非握手打点——
语音唤醒里，openvela 采集 48 kHz 音频、降采样后经 rpmsg 送往 A72，**Linux 侧的
k7d 做端点检测并回发整句区间**，openvela 再据此取段送识别。`ampctl exec` 可在
A72 上执行命令并回显，Linux 侧已具备完整用户态（initramfs + busybox）。

**当前的边界要讲明白**：端侧模型尚未部署，NPU 与 ISP 没有接入，所以现在的
"理解"仍由云端模型完成；Linux 侧承担的是端点检测这类计算，而不是推理。把
推理搬到 NPU 是下一步，不是已完成项。报告不把上层应用规划等同于已验收的
产品功能，也不用 SoC 的理论算力代替项目实测。

**对开发者一侧的问题定义**：在没有寄存器手册、上游没有任何 RK3576 代码的
前提下，先让系统可靠地访问硬件（启动、中断、时钟、外设），再保证显示、相机、
音频和通信同时工作时仍能响应用户，最后用大赛 xTS 用例逐条验收，并让每个结论
都能追到原始日志。三步的证据分别体现在 3.4、3.5 两节。
...
验收过程中发现并修复的三处上游缺陷：其一，`sched/task/task_posixspawn.c` 在找不到
...
其三，`drivers/sensors/bmi160_base.c` 在只开 I²C 不开 SPI 时编译不过（SPI 专用代码
未加条件编译），导致 1.3.7 的原文测试程序无法构建。三者分别归档在 `bsp/upstream/`，
可独立提交上游。
...
| MPU-6050 六轴传感器 | 40 针扩展口 I2C7：22 脚 SCL、24 脚 SDA、1 脚 3V3、6 脚 GND，AD0 接地取地址 0x68 | 替代原文的 BMI160 完成 xTS 1.3.7；板级驱动同时注册 uORB 与 `/dev/accel0` |

本项目涉及新平台 BSP 与驱动开发。GPIO 环回接线为扩展排针第 5 脚（GPIO4_A4）与第 7 脚（GPIO4_A6），1.3.6 复测 4/4，过程中修出一处芯片驱动缺陷：输出脚的读回走了 EXT_PORT，而本板该脚配置为输出时输入缓冲不工作，恒读 0，改读输出数据寄存器 SWPORT_DR 后通过。SPI 环回为第 10 脚与第 12 脚。正式提交须补实物照片和排针标注，按当前板卡手册确认电平，不能把芯片 GPIO 编号直接当成排针脚号。蓝牙物料识别在历史材料中存在 AP6256 与 SKW6621S 两种描述，最终 BOM 待实物和总线识别统一；目前仅将 SKW6621S SDIO 探测列为开发进展。
...
| B 部分通过 | **1** | 1.3.11 UART 文件传输：主机→板通过（md5 一致），板→主机受 1.5 Mbaud 下主机侧丢字（实测 7~16%）限制 |
| C 未达成 | **3** | 1.3.5、1.3.14、1.3.15 |

2026-09-20 当天把 B 档按原文重跑并补测：14 项升入 A，另有 1.3.1 烧写、1.3.6 GPIO 通过，其中 1.3.6 顺带修出一个 GPIO 驱动缺陷（输出脚读回读了 EXT_PORT，而本板该脚做输出时输入缓冲不工作，改读输出数据寄存器后 4/4）；1.1.4 ostest 的多线程锁子项要求四核都参与，在单系统镜像（`amp-solo.its`）上执行，`Exiting with status 0`、0 条失败关键字。

2026-09-20 傍晚又把 14 项 B 档按原文重跑（`notes/xts-rerun-raw/2026-09-20-btier/`，telnet + 串口双路取证）：调度 16/16、系统调用 82 项、scanf OK 164/0、pipe 4 项、fstest OK 20/0、UART 1/1、oneshot OK、八种加解密算法全过，均升入 A 档。

**1.3.7 I²C/SPI 由 C 转 A（2026-09-20 深夜）。** 原文指定 BMI160，手头没有，改用同为六轴、
寄存器语义一致的 MPU-6050 接在 40 针扩展口的 I2C7 上（22 脚 `I2C7_SCL_M1`、24 脚
`I2C7_SDA_M1`，AD0 接地取 0x68），先用 `i2c dev -b 7` / `i2c get -r 0x75` 确认总线上
确有 WHO_AM_I=0x68 的器件，再新增板级驱动：唤醒芯片、校验 WHO_AM_I 后同时注册 uORB
节点（`/dev/uorb/sensor_accel0`、`sensor_gyro0`）与字符节点 `/dev/accel0`。之所以要两个
节点，是因为原文命令 `cmocka_driver_i2c_spi` 读的是 BMI160 风格的 16 字节
`struct accel_gyro_st_s`，而 uORB 的阻塞读在只有 `fetch` 没有推送的驱动上会一直等信号量；
字符节点按测试期望的结构体直接返回。结果为 `[ PASSED ] 1 test(s)`，100 次读数中加速度 Z
约 −1.0 g（16384 LSB/g）与平放姿态一致，原始记录在 `notes/xts-rerun-raw/2026-09-20-mpu6050/`。
过程中还修出一处上游缺陷：`bmi160_base.c` 在只开 I²C 不开 SPI 时编译不过（SPI 专用代码未
加条件编译），补丁归档为 `bsp/upstream/bmi160-spi-only-guard.patch`。
...
软件伪随机数；补上开机 logo。另有三处上游缺陷在验收过程中被发现并修复，见 3.4.2。
...
| Token 使用总量 | 从 Claude Code 原始会话记录统计（本项目 3 个会话，模型 claude-opus-5），**按 `message.id` 去重后** 9,696 次回复：输出 **8,544,543**、缓存写入 58,885,426、新增输入 19,918、缓存读取 4,712,615,719；合计 **47.8 亿**（含缓存读取）或 **6,745 万**（不含）。缓存读取占 98.6%，来自每轮重读既有上下文（本会话上下文窗口 100 万 token，单次最高读取 99.9 万，平均 48.6 万），单价远低于新增输入；真正新产生的内容以输出计，平均每次回复 881 token；其中约 85%（≈730 万）是思考 token —— 日志只存签名不存思考文本，用字节数反推会对不上（可见正文 + 工具调用参数共 313 万字节，约 125 万 token）。衡量工作量更合适的口径是：9,696 次交互、2,960 次工具调用、约 125 万 token 的可见产出。★ 统计坑：一条流式回复在记录里会写入多次且各条带**同一份累计用量**，按行累加会多算约 2.3 倍（首次统计即因此得到 96.5 亿，已更正）。未折算费用，单价随套餐变化，不填推算值 |
...
1. **xTS 剩余 3 项**。1.3.14 需一段完整 24 h；1.3.15 需让复位原因可判——可行做法是看门狗首次超时进中断时写一处掉电才清的标志并主动 panic（原文注释正是这样要求厂商实现），board_reset_cause 再据此上报；1.3.5 需要一块可安全改写的 Flash 介质（板载 eMMC 上即启动镜像，不能作破坏性测试对象）。
