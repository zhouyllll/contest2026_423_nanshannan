...
| 1.3.13 | Timer 定时器 | ⛔ | **撤销之前的通过**。`/dev/oneshot` 与调度器抢同一个定时器下半部，注册它会打死整个系统的时基（见下）。已关闭，待给它一路独立的 RK3576 TIMER |
| 1.3.14 | 时间一致性 | ◆ | 无需配置，`date` 即可；要静置 24h。**必须在时基修好之后再测** |
| 1.3.15 | Watchdog | ⚠️ | 结论存疑：这一项是在时基已坏的状态下测的，用例里的等待可能根本没在计时，"触发复位"也许只是超时的自然结果。待复测 |
...
| 1.3.13 | **Timer 定时器** | `cmocka_driver_oneshot` | ⛔ 已关闭，见下 |
...
给 `/dev/oneshot` 一路**自己的**硬件定时器。RK3576 有独立的 TIMER 模块，
用其中一路做 oneshot 下半部，与调度器占用的 ARM 通用定时器互不相干。
在那之前宁可少一项 —— 整个系统的定时是坏的却记着一堆"通过"，比缺一项
糟糕得多。
