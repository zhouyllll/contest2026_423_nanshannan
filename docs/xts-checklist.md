...
```
GMAC1: 链路=已建立 自协商=已完成      ← 接交换机的那个口
GMAC0: 链路=未建立                    ← 未插线
RXCLK 两个口均有跳变；两个 probe 均返回 OK
```
...
与 PHY 自身时钟无关。于是表现成一个很有迷惑性的组合：

- PHY 寄存器读写完全正常、写进去能读回来
- BMCR 显示自协商已使能、无 power-down、无 isolate
- 但 ANER bit0 恒为 0（从未收到对端 FLP）、RXCLK 恒低、网口灯不亮
- 无论是两口对接还是接真交换机，都一样

关键的一条写入（照抄 `kernel-6.1/drivers/net/phy/maxio.c`）：

```c
write_paged(0xd92, 0x02, 0x200a);   /* 时钟模式 = 晶振 */
```

内核源码在它后面紧跟着打印 `clkmode(crystal)`。其余是模拟前端整定，
共 22 条分页写，顺序照抄不能改。

### 是怎么找到的

**把板子刷回原厂 Android，去读一个能工作的系统实际配了什么。**

- 启动日志报出 PHY 型号：`driver [MAE0621A-Q2C Gigabit Ethernet]`
  —— 匹配到的是**专用驱动**而非 Generic PHY，这本身就说明有厂商初始化
- `clk_summary`：refclko25m 25MHz 使能、125M、pclk=100MHz，与我们一致
- `pinmux-pins`：引脚组与我们的表逐一吻合，**且没有任何 25m_out 引脚**

最后一条同时纠正了我之前两轮的错误方向：我先据「dtsi 注释掉引脚组」
判断 PHY 自带晶振（对的），又因为看到 PHY 节点的 `clocks` 属性推翻它
（错的），还接连试了 m0/m1 两组引脚。**要证明一个外设该怎么配，
去看能工作的那个系统实际配了什么，不要从注释反推。**

### 本轮一并修掉的固件缺陷

- 引脚表只有 eth0m0 一份、pinmux 写死 `if (port == 0)` → GMAC1 引脚从未配置
- 时钟采样循环写死 GMAC0 的 bank → GMAC1 报的是 GMAC0 的引脚状态
- GMAC1 的 125M 分频在 **CLKSEL_CON(31)[4:0]**，且原先只在 port==0 时设置
- 补了 MDIO 写函数（原先只有读）
- 链路诊断只在失败分支 → 探测通过时反而什么都看不到

### 一处因果倒置的更正

「RXCLK 恒低」曾被我当成根因去查上游。实际上 **RGMII 的 RXCLK 是 PHY
在模拟前端工作起来之后才输出的**，它是结果不是原因。厂商初始化补上后，
RXCLK 自己就出现了。

### 待做

- 网络协议栈还没开（`ifconfig` command not found），当前只到链路层
- RGMII 延时线（`ioc_grf` 0x26040000，GMAC0 +0x6408/+0x640c、
  GMAC1 +0x6410/+0x6414，`tx_delay` 0x21/0x20，rgmii-rxid）尚未配置，
  收发数据前要补
