...
## ★ GMAC 现状：固件侧已排干净，卡在物理层

### 结论先行

本端 PHY 活着、寄存器配置正确、25MHz 参考时钟已实测到、自协商在跑，
**但线上收不到任何对端能量**（`LP(reg5)=0x0000`、`ANER(reg6) bit0=0`，
持续 6 秒，两个口都是）。剩下的排查在网线/走线这一侧。

### 已逐项证实的判据

| 判据 | 结果 |
|---|---|
| MDIO 扫描 0~31 | 仅 addr=0 应答，ID=`0x7b744411`，总线健康 |
| MDIO 写自检 | ANAR 写 `0x0141` 读回 `0x0141` |
| `BMCR=0x1140` | 自协商开、无 power-down、无 isolate |
| `BMSR=0x7949` | 正常的千兆 PHY 能力位 |
| 通告 | `ANAR=0x01e1`、`GBCR=0x0200`（1000-T 全双工） |
| 25M 参考时钟 | g3-4 / g2-30 焊盘均实测到跳变 |
| 两颗 PHY 复位 | 均已释放 |

### 本轮修掉的固件缺陷

- 引脚表只有 eth0m0 一份、pinmux 写死 `if (port == 0)` → GMAC1 引脚从未配置
  （MDIO 读回全 0 就是这个原因，现已能读到真实 PHY）
- 时钟采样循环写死 GMAC0 的 bank/pin → GMAC1 报的是 GMAC0 的引脚状态
- GMAC1 的 125M 分频在 **CLKSEL_CON(31)[4:0]**，不是 CON(30) 的相邻位域；
  且原先只在 port==0 时设置
- 从未使能 `REFCLKO25M_GMACn_OUT`，25M 输出引脚停在 GPIO 功能
- 补了 MDIO 写函数（原先只有读，做不了 PHY 层实验）

### 两条已更正的错误判断

1. **"RXCLK 由 PHY 输出，与链路无关"** —— 错。RGMII 的 RXCLK 是 PHY
   **在链路建立之后**才输出的，所以"无 RXCLK"是链路不通的*结果*而非原因。
   此前顺着它往上游查，方向是反的。
2. **"dtsi 注释掉了 25M 引脚组，说明 PHY 自带晶振"** —— 错。被注释的只是
   引脚组，时钟另有一条路径：PHY 节点的
   `clocks = <&cru REFCLKO25M_GMAC0_OUT>`，由 `dwmac-rk.c` 的
   `gmac_clk_enable()` 使能，且在放 PHY 复位之前。

排查全过程见 `notes/DEBUG-CASES.md` 案例 17。

### 待做

- 物理确认：网线是否到位；把一个口接已知可用的路由器/PC 做对照
- RGMII 延时线（`ioc_grf` 0x26040000，GMAC0 +0x6408/+0x640c、
  GMAC1 +0x6410/+0x6414，`tx_delay` 0x21/0x20，rgmii-rxid 关内部 RX 延时）
  —— 只影响数据采样，等链路起来再配，避免一次动多个变量
- `ifconfig` 报 command not found，网络应用层配置还没开
