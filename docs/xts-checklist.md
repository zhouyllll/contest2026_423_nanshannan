...
| 1.3.11 | UART 文件传输 | ◐ | **接收方向已完整验证**（主机 `sb --ymodem` → 板端 `rb -f /mnt`，逐字节一致）。发送方向仍不通，`rb -vv` 报 `Got 0103 sector header` —— 块号补码不对。**两个假设已被证伪**：① ONLCR 扩 `\n`（`sb` 不带 -d 时 recvfd=stdin/sendfd=stdout 虽是两个 fd 却指向同一设备，termios 按设备生效，对 stdin 设 raw 已经关掉了 OPOST）；② 块大小（`-k 128` 同样失败）。且单块小文件也报同样的 `0103`，**确定性说明是结构性问题而非随机丢字节**，根因未定位 |
